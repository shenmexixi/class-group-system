...
export default function GroupPage() {
  ...
  useEffect(() => {
    ...
    const checkAuth = () => {
      ...
      try {
        ...
        console.log('[checkAuth] 解析的用户:', user);
  ...
  // 获取分组数据
  useEffect(() => {
    console.log('[fetchGroupData] useEffect 触发');
    console.log('[fetchGroupData] currentUser:', currentUser);
    console.log('[fetchGroupData] mounted:', mounted);
    ...
    const fetchGroupData = async () => {
      try {
        // 添加时间戳参数防止缓存
        const url = `/api/groups/${encodeURIComponent(className)}?t=${Date.now()}`;
        console.log('[fetchGroupData] 请求 URL:', url);
        ...
        console.log('[fetchGroupData] API 返回数据:', data);
        console.log('[fetchGroupData] slots 数量:', data.slots?.length);
        console.log('[fetchGroupData] students 数量:', data.students?.length);
        console.log('[fetchGroupData] slots 详情:', JSON.stringify(data.slots, null, 2));
